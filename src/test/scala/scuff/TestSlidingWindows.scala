...
class TestSlidingWindows {
  ...
  private def counting(mp: MapProvider[Int]) {
    object Count extends Sum[Int] {
      override val default = Some(0)
    }
    val counts = SlidingWindow(Count, mp, Duration.Inf, 10.milliseconds, 100.milliseconds, 1.second)
    (1 to 50).foreach(i => counts.add(1, i))
    val count50ms = counts.snapshot(51).get()
    ...
    val count1s = counts.snapshot(1001).get()
    ...
    val count2100ms = counts.snapshot(2101).get()
  ...
  private def average(mp: MapProvider[(BigDecimal, Int)]) {
    val averages = SlidingWindow(Average[BigDecimal], mp, Duration.Inf, 10.milliseconds, 100.milliseconds, 1.second)
    (1 to 50).foreach(i => averages.add(i, i))
    val avg50ms = averages.snapshot(51).get()
    ...
    val avg1s = averages.snapshot(1001).get()
    ...
    val avg2100ms = averages.snapshot(2101).get()
    ...
    val avg5005ms = averages.snapshot(5005).get()
