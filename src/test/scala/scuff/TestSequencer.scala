...
class TestSequencer {
  ...
  @Test
  def `Normal sequence must work`(): Unit = {
    val seq = 1001L
  ...
  @Test
  def `Out-of-sequence must work`(): Unit = {
  ...
  def `Duplicates must be detected and an exception thrown`(): Unit = {
    val sequencer = new MonotonicSequencer[Long, java.lang.Long](consumer, 5)
    sequencer.apply(4L, 4L)
  }

  @Test
  def `Duplicates can be intercepted instead of throwing exception`(): Unit = {
    var ignored = 0L
  ...
  @Test
  def `Exception must be thrown when buffer capacity is exceeded`(): Unit = {
    val capacity = 10
  ...
  @Test
  def `Buffer capacity must expand`(): Unit = {
