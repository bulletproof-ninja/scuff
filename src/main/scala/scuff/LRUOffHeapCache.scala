...
/**
  ...
  */
final class LRUOffHeapCache[K, V](
    maxCapacity: Int,
    ser: Serializer[V],
    val defaultTTL: Duration = Duration.Zero, staleCheckFreq: FiniteDuration = 10.seconds, lock: ReadWriteLock = new ReentrantReadWriteLock)
  ...
  def store(key: K, value: V, ttl: Duration) = impl.store(key, toBuffer(value), ttl)
  ...
  def lookupOrStore(key: K, ttl: Duration)(constructor: => V): V = toValue(impl.lookupOrStore(key, ttl)(toBuffer(constructor)))
  def shutdown() = impl.shutdown()
  def lookupAndRefresh(key: K, ttl: Duration): Option[V] = impl.lookupAndRefresh(key, ttl).map(toValue)
  def refresh(key: K, ttl: Duration) = impl.refresh(key, ttl)
}
