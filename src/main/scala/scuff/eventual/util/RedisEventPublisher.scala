...
trait RedisEventPublisher[ID, EVT, CAT]
  ...
  protected def categories: Set[CAT]
  private lazy val defaultCategorizer = {
    if (categories.forall(_.isInstanceOf[java.lang.Enum[_]])) {
      new Categorizer {
    ...
    } else if (categories.forall(_.isInstanceOf[Enumeration#Value])) {
      new Categorizer {
      ...
      new Categorizer {
        val _categoryIndex = categories.toSeq.zipWithIndex.toMap
        def name(cat: CAT): String = cat.toString
  ...
  private lazy val pubChannels = {
    val channels = new Array[BinaryRedisPublisher[Transaction]](categories.size)
